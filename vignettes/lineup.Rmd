...
The first table is for samples in the first data set: each row is a
...
column is the sample in the second data set that has the highest
correlation.
...
However, there appear to be two pairs of sample mix-ups: (44,66)
and (24,92). The have low values for `selfc` and high values
for `maxc`, consistently between the two data sets.  But we don't know
whether the mix-ups are in the first or second data set.

In addition, sample 48 in the first data set appears to be much sample
...
thing you see when there is a sample duplicate...where sample 48 in
...
Let's drop sample 48 from the first data set.
...
expression data sets. We first find gene/genotype pairs that are
...
genome, for the QTL analysis (to assess genotype/expression
association). We assume a 0.2% genotyping error rate.
...
This gives us a warning about a small number of genes being > 2 Mbp
from any pseudomarker, but we can ignore this.

We now use `calc.locallod()` to perform a QTL analysis for each
expression trait. For each gene, we just look at the one location that
...
expression data sets, and do the analysis with matching individuals.
...
The `lod1` and `lod2` results are each a vector of `r length(lod1)`
LOD scores (one LOD score for each gene, calculated near its genomic
location). These LOD scores have highly skewed distributions, but
...
```{r disteg}
d_eg_1 <- disteg(f2cross, expr1[, lod1>25], pmar[lod1>25,], verbose=FALSE)
d_eg_2 <- disteg(f2cross, expr2[, lod2>25], pmar[lod2>25,], verbose=FALSE)
```
...
expression arrays, but here we're looking for small values rather than
large ones. In the first table, the rows correspond to genotype
samples; in the second table, the rows correspond to expression
arrays.
...
(65,68). These were concordant between the two expression data sets,
and so we conclude that the error was likely in the genotypes.
