...
98 individuals in each data set; there are two individuals missing
from each.
...
(`selfc`) is the self-self correlation. For the rows where `selfc`
is low but `maxc` is high, a sample
mix-up is indicated. The last column is the sample in the second data
set that has the highest correlation.
...
the maximum correlation to be small, and these cases it is.
...
This gives us a warning that a small number of genes being > 2 Mbp
