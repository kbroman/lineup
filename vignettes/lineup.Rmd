...
In addition, sample 48 in the first dataset appears to be much sample
76 in the second dataset, but sample 48 in the second data set doesn't
...
the first data set is perhaps a copy of sample 76 in the first dataset.
...
Let's drop sample 48 from the first dataset.
...
expression datasets, and do the analysis with matching individuals.

```{r calc_locallod, message=FALSE}
id1 <- findCommonID(f2cross, expr1)
lod1 <- calc.locallod(f2cross[,id1$first], expr1[id1$second,], pmar)
id2 <- findCommonID(f2cross, expr2)
lod2 <- calc.locallod(f2cross[,id2$first], expr2[id2$second,], pmar)
```

```{r disteg}
d_eg_1 <- disteg(f2cross, expr1[, lod1>25], pmar[lod1>25,])
d_eg_2 <- disteg(f2cross, expr2[, lod2>25], pmar[lod2>25,])
```
...
Mix-ups are (31,84), (65,68), and (24,92).

Recall that (24,92) was a mix-up between expr1 and expr2.
...
Mix-ups are (31,84), (65,68), and (44,66).

Recall that (44,66) was a mix-up between expr1 and expr2.


## Conclusions

In `expr1`, samples (24,92) were swapped. Also, 48 was a duplicate of
76.

In `expr2`, samples (44,66) were swapped.

In the genotype data, (31,84) were swapped, as were (65,68).
